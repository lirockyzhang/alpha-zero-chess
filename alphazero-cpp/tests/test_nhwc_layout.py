...
def test_nhwc_shape():
    """Test that encoding returns NHWC shape (8, 8, 119)."""
    fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
    encoding = alphazero_cpp.encode_position(fen)

    print(f"Encoding shape: {encoding.shape}")
    assert encoding.shape == (8, 8, 119), f"Expected (8, 8, 119), got {encoding.shape}"
    print("[PASS] NHWC shape correct: (8, 8, 119)")
...
def test_nhwc_memory_layout():
    ...
    # Expected strides: (8*119*4, 119*4, 4) for float32
    expected_strides = (8 * 119 * 4, 119 * 4, 4)
...
def test_nhwc_batch_encoding():
    ...
    # Verify shapes
    assert batch_buffer.shape == (batch_size, 8, 8, 119), \
        f"Expected batch shape ({batch_size}, 8, 8, 119), got {batch_buffer.shape}"
...
def test_nhwc_pytorch_compatibility():
    ...
    assert tensor_cl.shape == (1, 8, 8, 119), \
        f"Expected shape (1, 8, 8, 119), got {tensor_cl.shape}"
