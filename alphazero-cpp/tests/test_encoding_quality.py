...
def test_position_encoding_quality():
    ...
    # Check piece planes (first 12 planes)
    ...
    # white_pawns is shape (8, 8), so np.where returns (row_indices, col_indices)
    # Row indices are the ranks (0-7)
    ...
    # Check color plane (plane 14, not 12!)
    ...
    # Check castling plane (plane 16, not 14!)
    ...
    if pawn_count == 8 and black_pawn_count == 8 and color_value == 1.0:
        print("✓ PASS: Position encoding produces correct piece planes\n")
    else:
        print("✗ FAIL: Position encoding has incorrect values\n")
...
def test_move_encoding_coverage():
    ...
    for uci_move, description in test_cases:
        try:
            index = alphazero_cpp.move_to_index(uci_move)
            decoded = alphazero_cpp.index_to_move(index, fen)

            if decoded == uci_move:
                print(f"✓ {description}: {uci_move} → {index} → {decoded} (round-trip OK)")
            else:
                print(f"⚠ {description}: {uci_move} → {index} → {decoded} (round-trip mismatch)")
...
def test_perspective_flip():
    ...
    # From black's perspective, the board should be flipped
    # Black's pieces should be on "lower" ranks (from black's view)
    black_pawns = encoding_white[0]  # Current player's pawns (black)
    white_pawns = encoding_white[6]  # Opponent's pawns (white)

    black_pawn_ranks = np.where(black_pawns > 0)[0] // 8
    white_pawn_ranks = np.where(white_pawns > 0)[0] // 8

    print(f"✓ Black's view - Black pawn ranks: {np.unique(black_pawn_ranks)}")
    print(f"✓ Black's view - White pawn ranks: {np.unique(white_pawn_ranks)}")
    ...
    if color_value == 1.0:
        ...
        print("✗ FAIL: Perspective flip not working\n")
...
def test_batch_encoding():
    ...
    print(f"✓ Batch shape: {batch.shape} (expected: (3, 119, 8, 8))")
    ...
    if batch.shape == (3, 119, 8, 8) and diff_01 > 0 and diff_12 > 0:
        print("✓ PASS: Batch encoding works correctly\n")
    else:
        print("✗ FAIL: Batch encoding has issues\n")
