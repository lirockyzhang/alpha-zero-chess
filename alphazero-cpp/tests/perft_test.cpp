...
// Recursively generates all legal moves to a given depth and counts leaf nodes
uint64_t perft(Position& pos, int depth) {
    if (depth == 0) return 1;

    std::vector<Move> moves;
    MoveGenerator::generate_legal_moves(pos, moves);
...
int main() {
    ...
    std::cout << "=== AlphaZero Sync Chess Engine - Perft Validation ===" << std::endl;
    std::cout << std::endl;
