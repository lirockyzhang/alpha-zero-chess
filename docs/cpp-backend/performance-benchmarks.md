# C++ Backend Performance Benchmarks

## Latest Results: PyTorch channels_last Optimization (2026-02-02)

**Major breakthrough: 3x performance improvement with optimal PyTorch integration!**

### End-to-End MCTS Performance (with Real Trained Model)

Tested on RTX 4060 with 64-filter, 5-block trained AlphaZeroNetwork:

| Test | Performance | Details |
|------|-------------|---------|
| **Single Game (800 sims)** | **6,192.5 sims/sec** | 3.0x faster than baseline |
| **Multiple Games (400 sims)** | **10,009 sims/sec avg** | Peak: 11,995 sims/sec |
| **NN Throughput** | **10,292 evals/sec** | Optimized Tensor Core usage |
| **GPU Utilization** | **60.2%** | Excellent utilization |
| **Batch Encoding (256 pos)** | **435,843 pos/sec** | 2.22x OpenMP speedup |

### Performance Evolution

| Optimization Stage | Sims/sec | Improvement |
|-------------------|----------|-------------|
| Original baseline | 2,072 | - |
| First channels_last | 3,298 | +59% |
| Permute + to(device) | 3,563 | +72% |
| **Complete "input gate"** | **6,192** | **+199%** 🎯 |

### Key Optimization

The complete "input gate" pattern:
```python
...
See **[pytorch-integration.md](pytorch-integration.md)** for complete details.

---

## Previous Milestone: Phase 3 Complete

**All 3 critical issues have been successfully fixed and tested.** The AlphaZero C++ engine was ready for Phase 4 integration with the neural network.

Additionally, **Optimization #1 (OpenMP Parallel Batch Encoding)** was implemented and benchmarked, achieving a **5.45x speedup** and exceeding the <1ms target.
